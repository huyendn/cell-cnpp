...
neg_log_likelihood <- function(params, data) {
  ...
  if (any(f_neg1 < 0 | f1 < 0 | f0 < 0)) return(Inf)
  
  # Choose probability based on x
  epsilon <- 1e-12
  prob <- ifelse(x == 1, f1,
                 ifelse(x == 0, f0, f_neg1))
...
grad_neg_log_likelihood <- function(params, data) {
  ...
  c  <- params[3]; m1 <- params[4]; m2 <- params[5]
  ...
  # Derivatives
  df1_dp1 <- 1 / A
  df1_dc  <- -p1 * delta1^2 / A^2
  df1_dm1 <- -2 * p1 * c * delta1 / A^2
  
  df0_dp2 <- 1 / B
  df0_dc  <- -p2 * delta2^2 / B^2
  df0_dm2 <- -2 * p2 * c * delta2 / B^2
  
  # Derivatives for f_{-1}
  dfneg1_dp1 <- -df1_dp1
  dfneg1_dp2 <- -df0_dp2
  dfneg1_dc  <- -df1_dc - df0_dc
  dfneg1_dm1 <- -df1_dm1
  dfneg1_dm2 <- -df0_dm2
  ...
  dL_dc  <- -(I1 * df1_dc + I0 * df0_dc + Ineg1 * dfneg1_dc) / prob
...
# New initial parameters: p1, p2, c, m1, m2
init_params <- c(p1 = 0.3, p2 = 0.4, c = 0.01, m1 = 40, m2 = 60)

# Constraints for constrOptim: p1 > 0, p1 < 1, p2 > 0, p2 < 1, c > 0
ui <- rbind(
  c( 1,  0,  0, 0, 0),   # p1 > 0.001
  c(-1,  0,  0, 0, 0),   # p1 < 0.999
  c( 0,  1,  0, 0, 0),   # p2 > 0.001
  c( 0, -1,  0, 0, 0),   # p2 < 0.999
  c( 0,  0,  1, 0, 0),    # c > 1e-6
  c( 0,  0,  0, 1, 0),
  c( 0,  0,  0, 0, 1)
)
ci <- c(0.001, -0.999, 0.001, -0.999, 1e-6, 0, 0)
...
start <- c(p1 = 0.1, p2 = 0.1, c = 0.1, m1 = 1, m2 = 1)
...
colnames(res.prm) <- c("p1", "p2", "c", "m1", "m2")
dim(res.prm)
