...
Quantitative modeling of these processes can provide a framework to predict population behavior and discover intrinsic proliferative properties of stem cell self-renewal and differentiation ability. Deterministic models with ordinary differential equations are utilized to describe the temporal dynamics of cell population on average \cite{marciniak2009modeling, beretta2012mathematical, weekes2014multicompartment, chen2016overshoot}. However, since the dynamics of cell populations emerge from the stochastic behavior of individual stem cells, each of which may divide or differentiate at random times, probabilistic modeling approaches, including birth-death process, Gaussian jump model, and non-homogeneous Markov chain \cite{Tu2009, Ba2018, Sc2022}, are more verisimilar for describing cell proliferation process. Branching process provides a natural and mathematically rigorous framework for describing stochastic reproduction phenomena. In the classical Galton-Watson process and its extensions, each individual acts independently and gives rise to a random number of offspring according to specified probability distributions \cite{watson1875probability,harris1948branching, harris1963theory, asmussen1983branching}. The branching process models have become an important tool to investigate population size, growth and extinction in biology, epidemiology, and genetics. In the context of cell proliferation, branching process models have been applied to describe stem cell lineages, where each stem cell may be viewed as an independent reproducing unit. MacMillan et al. \cite{macmillan2011seeing} and Miguez \cite{miguez2015branching} both developed models under the branching process theory for the stem cell proliferation and differentiation during embryonic development. In these models, time is discretized using the average cell cycle length as the unit of generations. Instead of tracking individual cell divisions in continuous time, the model advances steps corresponding to the mean duration of a cell cycle. In addition, MacMillian et al. \cite{macmillan2011seeing} explored the multi-type branching process extension to simulate data for a cell proliferation with more than one cell type with proliferative potential.
...
This proliferation process can be naturally represented as a branching process, in which each viable stem cell acts as a branching individual producing “offspring” according to the division probabilities $p_j(t)$. Differentiated cells correspond to terminal offspring, and nonviable stem cells correspond to individuals that cannot further proliferate. When the number of viable stem cells $X_i$ is positive, a stem cell division will occur at time $T_i$, with interarrival times that are exponentially distributed at a rate proportional to $X_i$. Thus, given division times $T_i$, the change of viable stem cells, differentiated cells, and nonviable stem cells have the following marginal distributions
\begin{equation}
...
\begin{enumerate}
    ...
    For $t>u,$
    \[
       \operatorname{Cov}(X(t), X(u)) = S(t)S(u) r\int_0^u\frac{p_1(v) + p_3(v)}{S(v)}dv = \frac{S(t)}{S(u)} \operatorname{Var}(X(u)).
    \]
\end{enumerate}
\end{proposition}
...
\end{equation}
with probability 1.
At each arrival time, the number of stem cells can increase by at most one. As a consequence, the stochastic process $X(t)$ is dominated by the Yule process. Therefore, by the dominated convergence theorem  (see Siegrist (2021, Eq. (16.23.40) \cite{siegrist2021})) we obtain
\begin{equation}
\frac{d}{dt}E[X(t)]
...
\end{equation}
Finally, for $t>u$, the tower property and conditional expectation give
\begin{equation}
\begin{split}
E[X(t)X(u)]
...
\end{split}
\end{equation}
where $P(t)=\int_0^t[p_1(u)-p_3(u)]\,du$.
...
\begin{remark}
{\rm
Note that in Proposition~\ref{prop:moments} the division rate $r$ may be a deterministic function of time.
As long as the rate function $r(t)$ is sufficiently well behaved to allow the
application of the Dominated Convergence Theorem (for example, if $r(t)$ is continuous),
all derivations remain valid with only minimal adjustments. In particular, in the case
of a non-constant rate $r(t)$, the formulas for $S(t)$ and $V(t)$ are modified as follows:
\begin{equation*}
S(t)=S_0 \exp\!\left\{\int_0^t \bigl[p_1(u)-p_3(u)\bigr]r(u)\,du \right\},\quad\quad
V(t)=S(t)^2\int_0^t \frac{p_1(u)+p_3(u)}{S(u)}r(u)\,du.
\end{equation*}

However, for the data sets considered here, we believe that the assumption of a constant
division rate is appropriate. There are several reasons for this. First, our experimental
data are collected over relatively short time horizons (ranging from days to a few weeks).
Second, from a biological perspective, the choice of a specific functional form for $r(t)$
is not immediately clear. Third, allowing $r$ to depend on time would require introducing
additional parameters that are likely unnecessary for our model.

In fact, since we observe every arrival time $T_i$ and the corresponding stem cell count $X(T_i)$,
under the assumption of a constant division rate $r$ the random variables $X(T_i)\bigl(T_{i+1}-T_i\bigr)$
are independent and identically distributed with an exponential distribution of rate $r$.
This allows, for example, the use of goodness-of-fit tests to assess the validity of the
constant-rate assumption. We performed such tests for the two data sets analyzed in Section~\ref{application}
and found that the assumption of constant $r$ is supported by the data(see the Supplementary Materials).  

Nevertheless, we emphasize that if a non-constant division rate $r(t)$ is required,
the model readily accommodates this type of modification.
}\end{remark}

We validate the theoretical moments of stem cell count using simulated data. Details on how the data is simulated are discussed in section \ref{simulation}. Figure \ref{fig:sim_plot} (left) and table \ref{tab:autocorrelation} illustrate an example of empirical mean, variance and autocorrelation of stem cell counts obtained from simulated data. The empirical results closely match with their theoretical counterparts derived in proposition \ref{prop:moments}.
\begin{figure}[ht]
\centering
...
$p_2(t) = \frac{0.15}{1 + 0.012(t-12)^2}$,
$p_4(t) = \frac{0.20}{1+0.008(t-20)^2}$,
...
In practice, the division probabilities $p_1(t), p_2(t), p_3(t), p_4(t)$ are unknown and must be estimated using the observed cell proliferation data from experiments. In this section, we
 develop a likelihood-based framework for parameter estimation. When we observe all the events, both the timing of the events and the number of cells, we can write down the full likelihood of the observed data
...
\begin{figure}
    ...
    \caption{Diagnostic plots assessing the fit of the inverse Gaussian distribution to the simulated stopping times. The fitted distribution is based on parameters estimated using maximum likelihood.}
...
$$ (\log \LL_n)' = \sum_{k=0}^{n-1} \frac{d'_{k+1}}{d_{k+1}}.$$

\subsection*{Constant rate interarrival times for MEP divisions}
\label{sec:exp_test}

Under a constant per-cell division rate, the time between consecutive division events
follows an exponential distribution. Specifically, if $r$ is the division rate and $n$
is the current cell count, the interarrival time is exponential with rate $nr$; dividing
by $n$ and scaling by the sample mean yields a unit-rate exponential. We verify this
for each MEP dataset by computing scaled interarrival times as described, comparing
their empirical distribution to $\mathrm{Exp}(1)$ visually (Figure~\ref{fig:scale_arr_times_mep})
and via formal tests (Table~\ref{tab:interarrival_time_dist}).

\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{scale_arr_times_mep.png}
    \caption{Histograms of scaled interarrival times between MEP division events (grey bars)
    overlaid on the $\mathrm{Exp}(1)$ density (red curve), for the control condition (left)
    and the lacking TPO condition (right).}
    \label{fig:scale_arr_times_mep}
\end{figure}
...
\begin{table}[h]
    ...
    \begin{tabular}{lcccccc}
        \toprule
         & \multicolumn{2}{c}{Anderson--Darling}
         & \multicolumn{2}{c}{Cram\'{e}r--von Mises}
         & \multicolumn{2}{c}{Kolmogorov--Smirnov} \\
        \cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
         & $A_n$ & $p$-value & $\omega^2$ & $p$-value & $D$ & $p$-value \\
        \midrule
        Control condition      & 2.954 & 0.029 & 0.432 & 0.060 & 0.114 & 0.052 \\
        Lacking TPO condition  & 0.356 & 0.891 & 0.047 & 0.895 & 0.083 & 0.906 \\
        \bottomrule
    \end{tabular}
    \caption{Goodness-of-fit tests for the null hypothesis that the scaled interarrival
    times follow a constant-rate exponential distribution.}
    \label{tab:interarrival_time_dist}
\end{table}

For the lacking TPO condition, all three tests yield $p > 0.89$, strongly supporting
the exponential assumption. For the control condition the Anderson--Darling test gives
$p = 0.029$, while the Cram\'{e}r--von Mises and Kolmogorov--Smirnov tests give
$p = 0.060$ and $p = 0.052$, respectively. The Anderson--Darling test is more sensitive
to tail discrepancies; that it is the only test to cross the 0.05 threshold suggests
any deviation is confined to the tails rather than the bulk of the distribution.
Because the rate parameter is estimated from the same data, all $p$-values are slightly
liberal, so the borderline Anderson--Darling result should not be over-interpreted.
Overall, the constant-rate exponential assumption is well supported for both conditions.
