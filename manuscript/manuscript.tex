...
\end{equation}
with probability 1.
At each arrival time, the number of stem cells can increase by at most one. As a consequence, the stochastic process $X(t)$ is dominated by the Yule process ({\it insert here reference to Eq. (16.23.40) from Siegrist}).   Therefore, by the dominated convergence theorem we obtain
\begin{equation}
\frac{d}{dt}E[X(t)]
...
\end{equation}
with probability one. Taking expectations and letting $\Delta\to 0$ yields
\begin{equation}
\label{eq:Mprime}
M'(t)
